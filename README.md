...
- **设计软件兼容**：Photoshop/Illustrator 按 `T` 后点击画布或双击已有文字会进入中文输入态，退出编辑或点击普通界面恢复英文；Cinema 4D 只在名称重命名框中切中文。三款设计软件均绕过可能造成资源增长的 UIA 查询。
- **系统搜索兼容**：Windows 搜索框使用快速 Caret 检测，进入搜索框立即切换并显示状态。
- **应用规则窗口**：托盘“应用规则 (Apps)”列出当前已打开的可见应用，可选择自动识别输入框、固定中文、固定英文、忽略或删除规则。
- **单实例运行**：重复启动不会创建第二套托盘和检测线程。
