...
- **设计软件兼容**：Photoshop/Illustrator 分别跟踪画布文字与图层/对象重命名，点击文字工具或双击已有文字后进入中文；按 `Esc`、回车提交或点击普通区域后持续保持英文，只有再次进入文字编辑才切回中文。Adobe 启动阶段不发送输入语言消息，避免干扰启动。Cinema 4D、剪映和 CapCut 完全禁用自动切换和辅助功能探测，只提示手动切换后的当前状态。
...
- **应用规则窗口**：左键单击托盘图标即可打开居中的规则窗口；列表使用清晰小图标、完整行高亮和紧凑行距，图标与选中背景之间保留留白，不显示网页标题或文档内容，并避免横向滚动与按钮文字裁切。
