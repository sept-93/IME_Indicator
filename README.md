...
- **富文本兼容**：针对 Chromium 网页编辑器、微信、企业微信与飞书等自绘输入区，使用受限的 Caret 回退识别；中文切换未生效时会做一次延迟纠正。
- **设计软件兼容**：Photoshop/Illustrator 分别跟踪画布文字与图层/对象重命名，点击文字工具或双击已有文字后进入中文；按 `Esc`、提交重命名或切换 B/V 工具后恢复英文，并屏蔽退出后残留的假 Caret。Cinema 4D 完全禁用自动切换和辅助功能探测，只提示手动切换后的当前状态。
- **Office 编辑兼容**：WPS 文字、表格、演示以及 Microsoft Word、Excel、PowerPoint 使用 Caret 与双击状态联合检测；双击单元格或文字进入中文，点击非编辑区、按 Esc 或提交表格编辑后恢复英文。
- **系统搜索兼容**：Windows 搜索框使用快速 Caret 检测，进入搜索框立即切换并显示状态。
- **应用规则窗口**：左键单击托盘图标即可打开居中的规则窗口；列表使用较清晰的大图标、完整行高亮和更宽行距，只显示程序名，不显示网页标题或文档内容。
