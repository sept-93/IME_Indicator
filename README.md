...
- **设计软件兼容**：Photoshop/Illustrator 在文字工具点击画布或同一位置双击已有文字时进入中文输入态，按一次 `Esc` / `Ctrl+Enter` 或点击普通界面后恢复英文；Cinema 4D 完全禁用自动切换和辅助功能探测，只在用户手动切换输入法后显示当前状态。三款设计软件均绕过可能造成资源增长的 UIA 查询。
- **WPS 编辑兼容**：WPS 文字、表格和演示使用真实文本光标补充检测，双击进入单元格或文本编辑后可自动切换中文，退出编辑后恢复英文。
- **系统搜索兼容**：Windows 搜索框使用快速 Caret 检测，进入搜索框立即切换并显示状态。
- **应用规则窗口**：托盘“应用规则 (Apps)”以图标和程序名展示当前已打开的可见应用，不显示网页标题或文档内容，并用两行提供互不遮挡的规则选项。
...
Lark、WPS、Photoshop、Illustrator、Cinema 4D 和 Windows 搜索已内置兼容；其他应用如果有可见的文字光标但没有被识别，可从托盘打开“应用规则 (Apps)”并选择“自动识别输入框”，也可以手动把 EXE 文件名
