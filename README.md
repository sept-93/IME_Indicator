...
- **设计软件兼容**：Photoshop/Illustrator 分别跟踪画布文字与图层/对象重命名，点击文字工具或双击已有文字后进入中文；独立的高频鼠标按下沿记录避免漏掉快速双击。按 `Esc`、回车提交或点击普通区域后持续保持英文，只有再次进入文字编辑才切回中文。Adobe 启动阶段不发送输入语言消息，避免干扰启动。Cinema 4D、剪映和 CapCut 完全禁用自动切换和辅助功能探测，只提示手动切换后的当前状态。
