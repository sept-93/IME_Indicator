...
- **双击循环切换**：在微信、企业微信、浏览器和普通输入框/搜索框中，已聚焦可编辑区域可双击按当前真实状态在中文与英文之间循环；双击后的文字选择和后续单击定位不会立即覆盖该状态。详见 [自定义输入行为说明](./docs/custom-input-behavior.md)。
- **快速响应**：输入框、搜索框和 `contenteditable` 在识别到焦点的同一轮立即切换。
- **搜索框兼容**：针对焦点刚进入时尚未建立 IME 窗口的 WinUI/自绘搜索框，在最初 260ms 内有限复检；成功切换后立即停止，不影响后续手动一键切换。
- **富文本兼容**：针对 Chromium 网页编辑器、微信、企业微信与飞书等自绘输入区，使用受限的 Caret 回退识别；每个输入状态只自动处理一次，随后允许手动一键切换。
- **设计软件兼容**：Photoshop/Illustrator 分别跟踪画布文字、原生搜索框与图层/对象重命名，点击搜索框、文字工具或双击已有文字后进入中文；独立的高频鼠标按下沿记录避免漏掉快速双击。同一段画布文字内再次单击定位保持中文；按 `Esc`、回车提交或点击普通箭头区域后恢复英文。Adobe 启动阶段不发送输入语言消息，避免干扰启动。Cinema 4D、剪映和 CapCut 完全禁用自动切换和辅助功能探测，只提示手动切换后的当前状态。
...
- **应用规则窗口**：左键单击托盘图标即可打开居中的规则窗口；列表采用 26px 自绘行与原生 16px Shell 图标，图标不放大、不经过中间位图，并在完整行高亮内保留留白；不显示网页标题或文档内容，并避免横向滚动与按钮文字裁切。
