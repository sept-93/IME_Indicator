...
- **设计软件兼容**：Photoshop/Illustrator 分别跟踪画布文字与图层/对象重命名，点击文字工具或双击已有文字后进入中文；按 `Esc`、提交重命名或点击普通区域后优先恢复英文，退出动作不会再被残留 Caret 覆盖。Cinema 4D 完全禁用自动切换和辅助功能探测，只提示手动切换后的当前状态。
